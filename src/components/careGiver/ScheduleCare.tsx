...
const ScheduleCare = ({ isOpen, OnClose, selectedCaregivers }: ScheduleCareProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
      ...
      {/* Modal */}
      <div
        className="relative z-50 w-full max-w-lg bg-white rounded-3xl p-6 sm:p-8 shadow-lg"
        onClick={(e) => e.stopPropagation()}
      >
        <h1 className="text-center text-[var(--navy)] font-bold text-lg">
          Schedule Your Care
        </h1>
        <p className="text-center text-gray-400 text-md mt-1 mb-6">
        ...
        <div className="space-y-3 mb-10 max-h-40 overflow-y-auto pr-2">
          {selectedCaregivers.map((c, index) => (
            <div
              key={index}
              className="flex items-center border border-gray-300 rounded-lg p-2 space-x-3"
            >
              <img
                src={c.imgSrc}
                alt="avatar"
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[var(--navy)] font-semibold text-sm">{c.name}</p>
        ...
        >
          <div>
            <label className="text-[var(--navy)] font-semibold text-md">Care Type</label>
            <select
          ...
          <div className="flex space-x-3 mt-6">
            <button
              type="button"
              onClick={OnClose}
              className="flex-1 bg-gray-200 text-[var(--navy)] font-semibold text-sm rounded-full py-2 hover:bg-gray-300"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="flex-1 bg-yellow-400 text-[var(--navy)] font-semibold text-md rounded-full py-2 hover:bg-yellow-500"
            >
              Book Caregiver
            </button>
          </div>
