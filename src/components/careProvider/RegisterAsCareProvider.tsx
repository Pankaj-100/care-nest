...
const RegisterAsCareProvider = () => {
    return (
        <div className='flex h-auto'>
            <div className='relative min-h-[800px] w-1/2'>
                <Image src={'/register-care-provider.jpg'} alt="faq" fill style={{ objectFit: 'cover' }} />
            </div>
            <div className='p-20 flex justify-center bg-[var(--whiteSmoke)]'>
                <div>
                    <h1 className='text-2xl font-semibold  text-[var(--navy)]'>Start Caring, Start Earning – Register as a Caregiver Now</h1>

                    <form className='mt-8'>
                        {
                            inputsData.map((input, index) => (
                                <Input key={index} placeholder={input.placeholder} icon={input.icon} type={input.type} />
                            ))
                        }
                        <InputArea />
                        <YellowButton className='w-full mt-8'>
                            Submit
                        </YellowButton>
                    </form>
